...
(function (cx, logger, $) {
	...
	(function (header) {
		...
		//#region Variables
		//WebMail Count keeps track of the current WebMail count in the header. If a value is set externally, we'll take that value; otherwise, we'll initialize this value to 0.
		var webMailCount = header.webMailCount || 0;
		var ungradedAssessmentCount = header.ungradedAssessmentCount || 0;
		//#endregion
		...
		function setEventHandlers() {
			...
			//#region WebMail Count Update Event Handler
			//Set up event handler for webMailCountUpdate event.
			$(document).on('webMailCountUpdate', function(e, data) {
				log('Handling event webMailCountUpdate.');

				//The event contains a data payload with the WebMail count. Only continue if we have that count. 
				//We have to check for "!== 'undefined'" because "if (data.count)" does not hold true if the value is 0.
				if (typeof (data.count) !== 'undefined') {
					//Get references to the jQuery wrapped sets for reuse and performance.
					var $header = $('div.headerWrapper');
					var $mobile = $('a#mobileWebmailLink span.mobileNotification', $header);
					var $desktop = $('a#webmailLink span.notification', $header);

					//Only proceed if the new count differs from the previous count. If the counts are the same, 
					//we assume nothing changed and we don't perform any updates (especially regarding screen readers).
					if (cx.header.webMailCount !== data.count) {
						log('Current webMail count [' + cx.header.webMailCount + '] differs from new webMail count [' + data.count + ']. Proceeding to update header.');

						//Update the desktop & mobile count.
						cx.header.webMailCount = data.count;

						//Set the visibility of the desktop & mobile count. If we have a count, show it; otherwise, hide it (we don't show 0).
						if (data.count > 0) {
							//Only use the aria-live on desktop. Using it on mobile additionally would cause the screen reader to speak the same
							//announcement twice since DOM elements exist for both headers. Chris U. advised to use desktop over mobile.
							var notificationMarkup = '<span aria-hidden="true">' + data.count + '</span><span class="audibleOnly">' + data.count + (data.count !== 1 ? ' New Notifications' : ' New Notification') + '</span>';

							$desktop.html(notificationMarkup).show(); //~line 441 in headerBetaPartial.cshtml
							$mobile.html(notificationMarkup).show(); //~line 84 in headerBetaPartial.cshtml / ~line 35 in HeaderBeta.ascx / ~line 50 in IconBarBeta.ascx
						} else {
							$desktop.hide().text(data.count);
							$mobile.hide().text(data.count);
						}
					} else {
						log('Current webMail count [' + cx.header.webMailCount + '] does not differ from new webMail count [' + data.count + ']. No update needed.');
					}
				} else {
					log('Handling event webMailCountUpdate - *** NO DATA WAS PASSED - ABORTING ***');
				}
			});

			//#endregion

			//Set up event handler for ungradedAssessmentCountUpdate event.
			$(document).on('ungradedAssessmentCountUpdate', ungradedAssessmentCountUpdateHandler);
		}

		//#region Ungraded Assessment Count Update Event Handler
		function ungradedAssessmentCountUpdateHandler(e, data) {
			log('Handling event ungradedAssessmentCountUpdate.');

			if (typeof (data.count) !== 'undefined') {
				//Get references to the jQuery wrapped sets for reuse and performance.
				var $header = $('div.headerWrapper');
				var $mobile = $('a#mobileTeacherGradebookLink span.mobileNotification', $header);
				var $desktop = $('a#gradebookTeacherLink span.notification', $header);

				//Only proceed if the new count differs from the previous count. If the counts are the same, 
				//we assume nothing changed and we don't perform any updates (especially regarding screen readers).
				if (cx.header.ungradedAssessmentCount === data.count) {
					log('Current ungraded assessments count [' + cx.header.ungradedAssessmentCount + '] does not differ from new ungraded assessments count [' + data.count + ']. No update needed.');
					return;
				}

				log('Current ungraded assessments count [' + cx.header.ungradedAssessmentCount + '] differs from new ungraded assessments count [' + data.count + ']. Proceeding to update header.');

				//Update the desktop & mobile count.
				cx.header.ungradedAssessmentCount = data.count;

				//Set the visibility of the desktop & mobile count. If we have a count, show it; otherwise, hide it (we don't show 0).
				if (data.count > 0) {
					var notificationMarkup = '<span aria-hidden="true">' + data.count + '</span><span class="audibleOnly">' + data.count + (data.count !== 1 ? ' New Notifications' : ' New Notification') + '</span>';

					$desktop.html(notificationMarkup).show();
					$mobile.html(notificationMarkup).show();
				} else {
					$desktop.hide().text(data.count);
					$mobile.hide().text(data.count);
				}
			} else {
				log('Handling event ungradedAssessmentCountUpdate - *** NO DATA WAS PASSED - ABORTING ***');
			}
		}
		//#endregion
		...
		function toggleBetaHeader() {
			var cookieName = 'Header_Beta_Use';

			//If we have a cookie, remove it; otherwise, set it with a value of 1 (true)
			if (cx.utils.cookie.getCookie(cookieName).length > 0) {
				cx.utils.cookie.deleteCookie(cookieName);
			} else {
				cx.utils.cookie.setCookie(cookieName, 1);
			}

			//Reload the page, so that the server can render the correct header (based on the cookie).
			location.reload();
		}
		...
		// Public Variables
		header.webMailCount = webMailCount;
		header.ungradedAssessmentCount = ungradedAssessmentCount;

		// Public Methods
		header.docReady = docReady;
		header.goToMainContent = goToMainContent;
		header.setActiveState = setActiveState;
		header.toggleBetaHeader = toggleBetaHeader;
		header.setHomePageUrl = setHomePageUrl;
		header.ungradedAssessmentCountUpdateHandler = ungradedAssessmentCountUpdateHandler;
